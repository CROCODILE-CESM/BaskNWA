...
``` sh
$DART_ROOT_PATH/models/MOM6/work/create_fixed_network_obs_sequence < create_fixed_network_seq.in
```
...
1) my MOM6 history files that I want to re-interpolate to the Oleander grid are monthly files, so for each file I only need one month of observations (i.e. in step 6, more than 90\% of create_fixed_network_seq.out as generated above would be processed only to be discarded)

2) I will use model2obs to orchestrate the interpolation of the monthly files, and each monthly file contains daily averages, so only the observations from one day at the time will actually be used
...
This generated 365 files, each looking similar to this (e.g. create_fixed_network_seq_20230701.in):
``` text
create_obs_sequence.out                 # observation sequence file is the same for all
1
1                                       # each file will contain 1 replicate
2023 7 1 12 0 0                         # the datetime of the 1 replicate
1 0
create_fixed_network_seq_20230701.out   # name of single day file that will be generated by create_fixed_network_sq

```

To call DART create_fixed_network_sequence on each file there are a couple of options:

1) bash loop:

``` sh
for f in ./cfns_in/*; do "$DART_ROOT_PATH/models/MOM6/work/create_fixed_network_obs_sequence" < "$f"; done
```
